...
from utils import update_toml_file, load_toml_as_dict, save_dict_as_toml, api_base_url, cprint
...
class TrophyObserver:
    ...
    def add_trophies(self, game_result, current_brawler):
        ...
        cprint(f'Found game result: {game_result}', 'INFO')
        cprint(f'Win streak: {self.win_streak}', 'INFO')
        ...
        if game_result == 'victory':
            ...
            cprint('Nothing changed. Draw detected.', 'INFO')

        else:
            cprint('Detection suffered failure. May cause issues.', 'WARNING')

        cprint(f'Trophies: {old} -> {self.current_trophies}', 'INFO')
    ...
    def change_trophies(self, new):
        cprint(f'Trophies changed from {self.current_trophies} to {new}', 'INFO')
        ...
        if api_base_url != 'localhost':
            ...
            try:
                ...
                if response.status_code == 200:
                    cprint('Results successfully sent to API', 'CHECK')
                    ...
                    cprint(f'Failed to send results to API. Status code: {response.status_code}', 'FAIL')
            except requests.exceptions.RequestException as e:
                cprint(f'Error sending results to API: {e}', 'ERROR')