...
class SelectBrawler:
        ...
        if file_path:
            try:
                with open(file_path, 'r') as file:
                    ...
                    try:
                        if not brawlers_data[0]['push_until']:
                            raise ValueError('brawler_data -> push_until is empty')
                        for brawler_data in brawlers_data:
                            # if we find a brawler that has already reached its goal, we remove it from the list
                        ...
                        cprint('Brawler data loaded successfully.', 'CHECK')
                    except Exception as e:
                        cprint(f'Invalid data format. Expected a list of brawler data. {e}', 'WARNING')
            except Exception as e:
                cprint(f'Error loading brawler data: {e}', 'ERROR')
        linebreak()
    ...
    def open_brawler_entry(self, brawler):
        ...
        def submit_data():
            ...
            cprint(f'Selected Brawler Data: {self.brawlers_data}', 'INFO')
            cprint('Successfully updated data.', 'CHECK')
            top.destroy()
